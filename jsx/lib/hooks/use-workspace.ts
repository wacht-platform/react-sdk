import { Client, PaginatedResponse, Workspace, WorkspaceRole } from "@/types";
...
export const useWorkspaceList = () => {
        ...
        async (
            ...
            if (data.enable_ip_restriction !== undefined) {
                formData.append(
                    ...
                    String(data.enable_ip_restriction),
                );
            }
            if (data.whitelisted_ips) {
                data.whitelisted_ips.forEach((ip) =>
                    formData.append("whitelisted_ips", ip),
                );
            }
            const response = await responseMapper(
    ...
    const getWorkspaceMembers = useCallback(
        async (
            ...
            const response = await responseMapper<
                PaginatedResponse<WorkspaceMembership[]>
            >(
                await client(
    ...
    const deleteWorkspaceRole = useCallback(
        async (workspace: Workspace, role: WorkspaceRole) => {
            const response = await responseMapper(
                await client(
                    `/workspaces/${workspace.id}/roles/${role.id}/delete`,
                    {
                        method: "POST",
                    },
                ),
    ...
    const removeWorkspaceMember = useCallback(
        async (workspace: Workspace, memberId: string) => {
            const response = await responseMapper(
                await client(
                    `/workspaces/${workspace.id}/members/${memberId}/remove`,
                    {
                        method: "POST",
                    },
                ),
    ...
    const createWorkspaceInvitation = useCallback(
        async (
            ...
            const response = await responseMapper(
                await client(
                    `/organizations/${workspace.organization.id}/invitations`,
                    {
                        method: "POST",
                        body: formData,
                    },
...
export const useActiveWorkspace = () => {
    ...
    const createCurrentWorkspaceRole = useCallback(
        async (name: string, permissions: string[]) => {
            if (!activeWorkspace) return;
            return await createWorkspaceRole(
                activeWorkspace,
                name,
                permissions,
            );
    ...
    const createCurrentWorkspaceInvitation = useCallback(
        async (params: { email: string; workspaceRoleId?: string }) => {
            if (!activeWorkspaceWithOrg) return;
            return await createWorkspaceInvitation(
                activeWorkspaceWithOrg,
                params.email,
                params.workspaceRoleId,
            );
        },
    ...
    const discardCurrentWorkspaceInvitation = useCallback(
        async (invitationId: string) => {
            if (!activeWorkspaceWithOrg) return;
            return await discardWorkspaceInvitation(
                activeWorkspaceWithOrg,
                invitationId,
            );
        },
    ...
    const resendCurrentWorkspaceInvitation = useCallback(
        async (invitationId: string) => {
            if (!activeWorkspaceWithOrg) return;
            return await resendWorkspaceInvitation(
                activeWorkspaceWithOrg,
                invitationId,
            );
        },
