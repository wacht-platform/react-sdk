...
export function useSession(): UseSessionReturnType {
    ...
    return {
        ...
        switchSignIn: async (signInId: string) => {
            ...
            await globalMutate("/me/organization-memberships", undefined, {
                revalidate: false,
            });
            await globalMutate("/me/workspace-memberships", undefined, {
                revalidate: false,
            });
            await mutate(undefined, { revalidate: true });
        ...
        signOut: async (signInId?: string) => {
            ...
            if (cache instanceof Map) {
                ...
                for (const key of keys) {
                    ...
                    if (staticKeys.includes(key as string)) {
                        ...
                        if (
                            dynamicKeyPrefixes.some((prefix) =>
            ...
            if (deployment?.ui_settings) {
                if (
                    signInId &&
                    deployment.ui_settings.after_sign_out_one_page_url
                ) {
                    navigate(
                        deployment.ui_settings.after_sign_out_one_page_url,
                    );
                    ...
                    !signInId &&
                    deployment.ui_settings.after_sign_out_all_page_url
                ) {
                    navigate(
                        deployment.ui_settings.after_sign_out_all_page_url,
                    );
