...
export const TwoFactorIcon = styled.div<{ $active?: boolean }>`
    ...
    box-shadow: ${(props) =>
        props.$active ? "var(--shadow-success)" : "var(--shadow-sm)"};
...
export const BackupCodeGrid = styled.div`
    ...
    grid-template-columns: repeat(
        auto-fill,
        minmax(calc(var(--space-10u) * 6), 1fr)
    );
    gap: var(--space-4u);
`;
...
export const BackupCodeItem = styled.button<{ $used?: boolean }>`
    ...
    border: var(--border-width-thin) solid
        ${(props) =>
            props.$used ? "var(--color-border)" : "var(--color-border)"};
    ...
    svg {
        ...
        color: ${(props) => {
            switch (props.$type) {
                ...
                default:
                    return "var(--color-card-foreground)";
            }
        }};
    }
`;
...
export const ActiveAuthenticator = styled.div`
    background: linear-gradient(
        135deg,
        var(--color-success-background),
        var(--color-success-background-light)
    );
...
export const SetupStepIndicator = styled.div<{
    $active?: boolean;
    $completed?: boolean;
}>`
    width: ${(props) =>
        props.$active ? "calc(var(--size-8u) * 2)" : "var(--space-4u)"};
    height: var(--space-4u);
